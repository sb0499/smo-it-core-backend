...
export const createActivo = async (data: {
  ...
  especificaciones?: string; persona_id?: number; fecha_compra?: string;
}) => {
  const estado = data.persona_id ? 'Asignado' : 'Stock';
  const [result] = await pool.query<ResultSetHeader>(
    `INSERT INTO activo (codigo, serial, marca, modelo, especificaciones, estado, persona_id, fecha_compra)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [data.codigo, data.serial, data.marca, data.modelo,
     data.especificaciones || null, estado, data.persona_id || null, data.fecha_compra || null]
  );
  const [rows] = await pool.query<RowDataPacket[]>(`SELECT * FROM activo WHERE id = ?`, [result.insertId]);
  return rows[0];
};

export const asignarActivo = async (activoId: number, personaId: number, usuarioAutorizaId: number) => {
  ...
  await pool.query(
    `INSERT INTO movimiento_inventario (activo_id, desde_persona_id, hacia_persona_id, usuario_id, tipo, observaciones)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [activoId, personaAnteriorId, personaId, usuarioAutorizaId, tipo, 'Movimiento generado por el sistema']
  );
